...
const ContaCorrente = () => {
    ...
    const [interestStepPercent, setInterestStepPercent] = useState(
        DEFAULT_INTEREST_STEP_PERCENT
    );
    const [interestStepMonths, setInterestStepMonths] = useState(
        DEFAULT_MONTHS_STEP
    );
        ...
        async ({ background = false } = {}) => {
            ...
            try {
                const response = await api.get(
                    `/conta-corrente/${clienteId}${buildQueryString()}`
                );
    ...
    const taxaJurosEfetiva = useCustomRate
        ? safeNumber(customMonthlyRate)
        : taxaJurosCadastro || 3;
        ...
        <Layout>
            ...
            <div className={styles.header}>
                <div className={styles.info}>
                    ...
                    <p>
                        <strong>Risco:</strong> {riscoCredito} |{' '}
                        <strong>Juros Cadastro:</strong> {taxaJurosCadastro}% a.m.
                    </p>
                </div>
            </div>
            ...
            >
                ...
                <div
                    className={styles.balanceCard}
                    style={{ backgroundColor: '#ecfeff', color: '#333' }}
                >
                ...
                >
                    <div>
                        ...
                        >
                            ...
                            <strong style={{ fontSize: '16px' }}>Perfil do Devedor</strong>
                        </div>
                            ...
                            <>
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setInterestStepPercent(Math.max(0, safeNumber(e.target.value)))
                                        }
                                        style={{
                                            width: '100%',
                                            padding: '10px 12px',
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setInterestStepMonths(Math.max(1, safeNumber(e.target.value)))
                                        }
                                        style={{
                                            width: '100%',
                                            padding: '10px 12px',
                        ...
                        <div style={{ color: '#64748b', fontSize: '13px' }}>
                            {interestMode === 'FIXO' ? (
                                ...
                                <>
                                    ...
                                    <strong>{interestStepPercent}%</strong> a cada{' '}
                                    <strong>{interestStepMonths} meses</strong>.
                                </>
                            )}
                        </div>
                    </div>
                        ...
                        {extrato.map((transacao) => (
                            <tr key={transacao.id} className={getRowStyle(transacao)}>
                                ...
                                <td style={{ textAlign: 'right' }}>
                                    {safeNumber(transacao.peso_kg) > 0
                                        ? `${formatKg(transacao.peso_kg)}`
                                        : '-'}
                                </td>

                                <td style={{ textAlign: 'right' }}>
                                    {safeNumber(transacao.preco_por_kg) > 0
                                        ? formatCurrency(transacao.preco_por_kg)
                                        : '-'}
                                </td>

                                <td style={{ textAlign: 'right', fontWeight: 'bold' }}>
                                    {renderValueCell(transacao)}
                                </td>
